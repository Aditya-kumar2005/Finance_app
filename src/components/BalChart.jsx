...
export default function BalChart() {
  const{st}=useTxn();const{dark}=useTheme();
    const ref=useChart(canvas=>{
        ...
        return new Chart(canvas,{type:'line',data:{labels:dl,datasets:[{label:'Balance',data,borderColor:'#10B981',backgroundColor:g,fill:true,tension:.4,pointRadius:5,pointBackgroundColor:'#10B981',pointBorderColor:dark?'#0F172A':'#FFF',pointBorderWidth:3,pointHoverRadius:7,borderWidth:2.5}]},options:{responsive:true,maintainAspectRatio:false,interaction:{intersect:false,mode:'index'},plugins:{legend:{display:false},tooltip:{backgroundColor:c.tip,titleColor:c.tipT,bodyColor:c.tipT,borderColor:c.tipB,borderWidth:1,cornerRadius:10,padding:12,bodyFont:{family:'DM Sans',weight:'600'},callbacks:{label:x=>'Balance: '+fmtCur(x.raw)}}},scales:{x:{grid:{display:false},ticks:{color:c.text,font:{family:'DM Sans',size:11}}},y:{grid:{color:c.grid},ticks:{color:c.text,font:{family:'DM Sans',size:11},callback:v=>fmtCur(v)},border:{display:false}}}}});
    },[st.txns,dark]);
    return <div className="lg:col-span-3 cd"><h3 className="font-heading font-semibold text-base mb-4">Balance Trend</h3><div className="relative" style={{height:280}}><canvas ref={ref}/></div></div>;
}
