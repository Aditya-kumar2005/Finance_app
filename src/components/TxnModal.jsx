...
import { EXP_CATS } from './Constant';

export default function TxnModal({open,mode,editId,onClose}){
    const{st,disp}=useTxn();const{add}=useToast();
    ...
    useEffect(()=>{
        if(!open)return;
        setErr('');
        if(mode==='edit'&&editId){const t=st.txns.find(x=>x.id===editId);if(t){setDesc(t.description);setAmt(String(t.amount));setDate(t.date);setTp(t.type);setCat(t.category);}}
        else{setDesc('');setAmt('');setDate(new Date().toISOString().split('T')[0]);setTp('expense');setCat(EXP_CATS[0]);}
    },[open,mode,editId]);

    useEffect(()=>{if(open&&dRef.current)setTimeout(()=>dRef.current.focus(),50);},[open]);

    const cats=tp==='income'?INC_CATS:EXP_CATS;
    useEffect(()=>{if(!cats.includes(cat))setCat(cats[0]);},[tp]);
    ...
    const submit=e=>{
        ...
        if(!desc.trim()||isNaN(a)||a<=0||!date||!cat){setErr('Please fill in all fields with valid values.');return;}
        if(mode==='edit'&&editId){disp({type:A.UPD,p:{id:editId,d:{description:desc.trim(),amount:a,date,type:tp,category:cat}}});add('Transaction updated','success');}
        else{disp({type:A.ADD,p:{id:nextId(st.txns),description:desc.trim(),amount:a,date,type:tp,category:cat}});add('Transaction added','success');}
        onClose();
    };
